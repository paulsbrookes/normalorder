...
class Operator(SortedDict):
    ...
    def __init__(self, spec=None, n_modes=None, safe=True):
    ...
    def __single_mode_mul__(self, other):
        ...
        for exponents_1, coeff_1 in self.items():
            for exponents_2, coeff_2 in other.items():
                ...
                op_product = Operator({}, n_modes=1, safe=False)
    ...
    def get_term(self, key):
        coeff = self[key]
        term = Operator({key: coeff}, n_modes=self.n_modes, safe=False)
        return term
        ...
        for exponents_1, coeff_1 in self.items():
            for exponents_2, coeff_2 in other.items():
                ...
                for mode_idx in range(self.n_modes):
                    component_1 = Operator({exponents_1[2 * mode_idx:2 * (mode_idx + 1)]: 1.0}, n_modes=1, safe=False)
                    component_2 = Operator({exponents_2[2 * mode_idx:2 * (mode_idx + 1)]: 1.0}, n_modes=1, safe=False)
